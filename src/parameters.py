...
class TrainingParameters(BaseModel):
    ...
    # num_workers: Optional[int] = Field(None, description="number of workers") # Commented out in case we need these back in the future
    # pin_memory: Optional[bool] = Field(None, description="memory pinning") # Commented out in case we need these back in the future
...
class SMSNetEnsembleParameters(TrainingParameters):
    num_networks: Optional[int] = Field(default=3, description='how many networks will be used in the ensemble')
    layers: Optional[int] = Field(default=5, description="number of layers, limit from 5 to 20")
